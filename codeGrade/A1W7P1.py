...
# Function to calculate average temperature per month
...
# Function to calculate average temperature per year
def average_temp_per_year(temperatures: dict) -> list:
    ...
    for year, months in temperatures.items():
        ...
        truncated_avg_temp = int(avg_temp * 100) / 100
        avg_temps_per_year.append((year, truncated_avg_temp))
    return avg_temps_per_year


# Function to print the warmest and coldest year based on average temperature
...
# Function to print the warmest/coldest month of a year
...
# Function to print a list of tuples (year, {month: average_temperature_in_celsius})
def average_temp_per_month_in_celsius(temperatures: dict) -> list:
    ...
    for year, months_data in temperatures.items():
        ...
        for month, temps in months_data.items():
            ...
            truncated_avg_f = int(avg_f * 100) / 100
            avg_c = fahrenheit_to_celsius(truncated_avg_f)
...
def main():
    ...
    if choice == "1":
        ...
        output = ", ".join([f"({year}, {avg_temp})" for year, avg_temp in avg_temps])
        print(f"[{output}]")
        ...
        year = int(input("Enter the year: "))
        coldest_month = extreme_month_of_year(temperatures, year, warmest=False)
        ...
        for year, months_data in avg_temps_celsius:
            print(f"({year}, {{", end="")
            month_strings = []
            for month, avg_temp_c in months_data.items():
                month_strings.append(f"{month}: {avg_temp_c:.2f}[0-9]*")
