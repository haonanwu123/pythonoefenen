...
def get_pizza_preferences(num_people):
    ...
    for i in range(1, num_people + 1):
        ...
        is_vegetarian = input(
            "Does this person eat only vegetarian pizzas? (yes/no): ").strip().lower() == 'yes'

        available_pizzas = VEGETARIAN_PIZZAS if is_vegetarian else VEGETARIAN_PIZZAS + \
            NON_VEGETARIAN_PIZZAS
        print(f"Available pizzas: {', '.join(available_pizzas)}")

        person_prefs = []
        for j in range(3):
            while True:  # Loop until we get a valid input or the user skips
                pizza_pref = input(f"Enter preference {
                                   j + 1} (or press Enter to skip): ").strip()
                if pizza_pref == "":
...
def distribute_pizzas(preferences, veg_slices, non_veg_slices, vegetarian_people, veg_pizzas, non_veg_pizzas):
    ...
    pizza_distribution = {person: []
                          for person in preferences}  # Track distribution
    # Track how many of each pizza type is distributed
    pizza_count = {
        pizza: 0 for pizza in VEGETARIAN_PIZZAS + NON_VEGETARIAN_PIZZAS}
    ...
    remaining_people = total_people - \
        len([p for p in preferences if preferences[p]]
    ...
    if remaining_people > 0:
        ...
        if vegetarian_people > 0 and veg_slices > 0:
            ...
            slices_per_veg_person, leftover_veg_slices = distribute_slices(
                veg_slices, remaining_veg_people)
            for person in pizza_distribution:
                if not preferences[person]:  # No preferences
                    # Give them a default vegetarian pizza
                    pizza_distribution[person].extend(
                        [VEGETARIAN_PIZZAS[0]] * slices_per_veg_person)
        ...
        if non_veg_slices > 0:
            slices_per_non_veg_person, leftover_non_veg_slices = distribute_slices(
                non_veg_slices, remaining_non_veg_people)
            for person in pizza_distribution:
                if not preferences[person]:  # No preferences
                    # Give them a default non-vegetarian pizza
                    pizza_distribution[person].extend(
                        [NON_VEGETARIAN_PIZZAS[0]] * slices_per_non_veg_person)
...
def main():
    ...
    slices_per_pizza = int(
        input("How many slices does each pizza have? (e.g., 4, 6, 8, or 12): "))
    ...
    pizza_distribution, leftover_veg_slices, leftover_non_veg_slices = distribute_pizzas(
        preferences, veg_slices, non_veg_slices, vegetarian_people, veg_pizzas, non_veg_pizzas)

    # Display results
    print("\nPizza Distribution Results:")
    for person, pizzas in pizza_distribution.items():
        pizza_count = {pizza: pizzas.count(pizza) for pizza in set(
            pizzas)}  # Count each type of pizza for the person
        print(f"{person}: Received - {', '.join(
            [f'{pizza} ({count} slices)' for pizza, count in pizza_count.items()])}")
    ...
    for person, prefs in preferences.items():
        print(f"{person}: Preferences - {', '.join(prefs)
              if prefs else 'No preferences'}")

    print(f"\nTotal pizzas: {num_pizzas} (Vegetarian: {
          veg_pizzas}, Non-vegetarian: {non_veg_pizzas})")
    print(f"Total slices: {total_slices} (Vegetarian slices: {
          veg_slices}, Non-veg slices: {non_veg_slices})")
